...
def validate_revenue(db: Session) -> dict:
    ...
    # Check for pence errors: item_price > 500 likely means value was in pence
    pence_errors = [o for o in orders if o.item_price > 500]
    if pence_errors:
        issues.append(
            f"{len(pence_errors)} orders may have prices in pence (item_price > 500)"
        )
        confidence -= 20 * len(pence_errors)

    # Check for calculation errors: revenue_estimate > item_price * quantity * 1.1
    calc_errors = [
        o for o in orders
        if o.revenue_estimate > o.item_price * o.quantity * 1.1
    ]
    if calc_errors:
        issues.append(
            f"{len(calc_errors)} orders have revenue_estimate exceeding expected calculation"
        )
        confidence -= 15 * len(calc_errors)
